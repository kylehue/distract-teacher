...
describe("lib/datetime", () => {
   ...
   it("handles invalid timestamp comparisons deterministically", () => {
      ...
      expect(compareTimestamps("invalid-a", "2025-01-01T00:00:00.000Z")).toBe(
         -1,
      );
      expect(compareTimestamps("2025-01-01T00:00:00.000Z", "invalid-b")).toBe(
         1,
      );
   });
   ...
   it("computes totalTime", () => {
      expect(
         totalTime("2025-01-01T00:00:00.000Z", "2025-01-01T00:00:30.000Z"),
      ).toBe("30s");
      expect(
         totalTime("2025-01-01T00:00:00.000Z", "2025-01-01T01:02:03.000Z"),
      ).toBe("1h 2m");
   });

   it("returns 0s for zero, negative, and invalid time deltas", () => {
      expect(
         totalTime("2025-01-01T00:00:00.000Z", "2025-01-01T00:00:00.000Z"),
      ).toBe("0s");
      expect(
         totalTime("2025-01-01T00:01:00.000Z", "2025-01-01T00:00:00.000Z"),
      ).toBe("0s");
